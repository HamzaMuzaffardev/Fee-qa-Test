...
// 
const targetLabel = document.querySelector('label[for="template--20069970641093__main-2-0"]');
const targetSection = document.querySelector('.custom-measurements-wrapper');

function checkColorAndToggle() {
  if (!targetLabel || !targetSection) return;

  // Get the computed background color
  const computedStyle = window.getComputedStyle(targetLabel);
  const bgColor = computedStyle.backgroundColor;

  // Get the value of the CSS variable to compare against
  const darkColor = window.getComputedStyle(document.documentElement)
    .getPropertyValue('--color-dark').trim();

  // Create a temporary element to resolve the CSS variable to RGB if necessary
  const tempElem = document.createElement('div');
  tempElem.style.color = `var(--color-dark)`;
  document.body.appendChild(tempElem);
  const resolvedDarkColor = window.getComputedStyle(tempElem).color;
  document.body.removeChild(tempElem);

  // Compare and toggle display
  if (bgColor === resolvedDarkColor) {
...
// 2. Run when the label is clicked (assuming interaction changes the state)
targetLabel.addEventListener('click', () => {
  // Use a slight timeout to allow the CSS transition/state change to finish
  setTimeout(checkColorAndToggle, 50);
});

// 3. Optional: Observe changes if the color is changed by other scripts
const observer = new MutationObserver(checkColorAndToggle);
observer.observe(targetLabel, { attributes: true, attributeFilter: ['style', 'class'] });